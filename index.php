...
foreach( $xml->children() as $child )
{
	switch( $child->getName() )
	{
		...
		case("earlyaccess"):
			...
			{
				$earlyaccess[$i][$subchild->getName()] = $subchild;
				$i++;
			}
			break;
...
<title><?php echo(GAME_TITLE); ?> Presskit</title>
    ...
    <div id="content">
        ...
        <?php 
			for( $i = 0; $i < count($platforms); $i++ )
			{
				...
				echo( '<a href="http://www.'.parseHubLink($link).'/">'.$name.'</a><br/>' );
			}
		?>
        </p>
        ...
        <table>
        <?php
			for( $i = 0; $i < count($earlyaccess); $i++ )
			{
				foreach( $earlyaccess[$i]['price']->children() as $child )
				{
        ...
        <?php
			...
			closedir($handle);
				...
				echo('<p style="clear:both">There are far more images available for '.GAME_TITLE.', but these are the ones we felt would be most useful to you. If you have specific requests, please do <a href="#contact" target="_self">contact us</a>!</p>');
